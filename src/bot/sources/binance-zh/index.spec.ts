...
describe('binance', ()=>{
  ...
  it('It detects multiple coins in multiple anouncement', async ()=>{
    const article1Mock = createArticle('Binance Will List Spell Token (SPELL) and TerraUSD (UST)', Date.now())
  ...
  it('It discards old announcements', async ()=>{
    const articleDateMock = Date.now() - config.discardSignalsOlderThanMilis - 1
